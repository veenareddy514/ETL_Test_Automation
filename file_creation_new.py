...
# ------------------------
# Orders Target (SCD Type 2) → Keep history, use is_active flag
# ------------------------
# Load 1
orders_target_load1 = orders_staging_df.copy()
orders_target_load1["batch_date"] = "2025-12-15"
orders_target_load1["is_active"] = 'Y'

# Load 2 → Type 2 change for order_id 103
orders_target_load2 = orders_staging_df.copy()
orders_target_load2["batch_date"] = "2025-12-16"
orders_target_load2.loc[orders_target_load2["order_id"] == 103, "amount"] = 310
# Previous record for order_id 103 → inactive
orders_target_load1.loc[orders_target_load1["order_id"] == 103, "is_active"] = 'N'
orders_target_load2["is_active"] = 'Y'

# Load 3 → Type 2 change for order_id 105
orders_target_load3 = orders_staging_df.copy()
orders_target_load3["batch_date"] = "2025-12-17"
orders_target_load3.loc[orders_target_load3["order_id"] == 105, "amount"] = 620
# Previous record for order_id 105 → inactive
orders_target_load1.loc[orders_target_load1["order_id"] == 105, "is_active"] = 'N'
orders_target_load3["is_active"] = 'Y'

# Combine all orders target loads
orders_target_df = pd.concat([orders_target_load1, orders_target_load2, orders_target_load3], ignore_index=True)
